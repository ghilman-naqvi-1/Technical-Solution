...
//         // Set initial state
//         gsap.set('.choosing-box', {opacity: 0, y: 50});
        
//         // Create timeline for project cards
//         const projectsTl = gsap.timeline({
//             scrollTrigger: {
//                 trigger: choosingContainer,
//                 start: 'top 90%',
//             }
//         });
        
//         // Add each card to the timeline
//         document.querySelectorAll('.choosing-container').forEach((card, index) => {
//             projectsTl.to(card, {
//                 opacity: 1,
//                 y: 0,
//                 duration: 0.8,
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    servicesItems.forEach(selector => {
        gsap.from(selector, {
            scrollTrigger: {
                trigger: '#Services',
                start: 'top 80%',
    ...
    /* ---------------------------
       CHOOSING SECTION TEXT
    --------------------------- */
    const choosingSection = document.querySelector('.choosing-section');

    if (choosingSection) {
        const textItems = ['.section-subtitle', '.section-desc'];

        gsap.set(textItems, { opacity: 0, y: 50 });

        const tlChoosingText = gsap.timeline({
            scrollTrigger: {
                trigger: choosingSection,
                start: 'top 85%',
            }
        ...
        textItems.forEach((selector, index) => {
            tlChoosingText.to(selector, {
                opacity: 1,
                y: 0,
                duration: 0.8,
                clearProps: "all"
    ...
    if (choosingContainer) {
        ...
        const tlChoosingBoxes = gsap.timeline({
            scrollTrigger: {
                trigger: choosingContainer,
                start: 'top 90%',
            }
        ...
        document.querySelectorAll('.choosing-box').forEach((box, index) => {
            tlChoosingBoxes.to(box, {
                opacity: 1,
                y: 0,
                duration: 0.8,
                clearProps: "all"
